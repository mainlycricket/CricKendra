			...
			SELECT
				ARRAY_AGG (
					...
					ROW (
						...
						innings.total_runs,
						innings.total_balls / matches.balls_per_over + (innings.total_balls % matches.balls_per_over) * 0.1,
						innings.total_wickets, innings.innings_end, innings.target_runs, innings.max_overs
					)
				)
		...
		WITH
				...
				SELECT
					...
					ROW_NUMBER() OVER (
						...
						ORDER BY
							...
							bs.balls_bowled DESC, bs.maiden_overs, bs.bowling_position ASC
					) AS rn
				FROM
			...
			scorecard_summary AS (
				SELECT
					...
					innings.total_runs, innings.total_wickets, innings.total_balls / matches.balls_per_over + (innings.total_balls %% matches.balls_per_over) * 0.1,
				...
				GROUP BY
					...
					innings.total_runs, innings.total_wickets, innings.total_balls, matches.balls_per_over
			),
				...
				SELECT
					ARRAY_AGG (
						...
						ROW (
							...
							innings.total_runs, innings.total_balls / matches.balls_per_over + (innings.total_balls %% matches.balls_per_over) * 0.1, innings.total_wickets, innings.byes, innings.leg_byes, innings.wides, innings.noballs, innings.penalty,
