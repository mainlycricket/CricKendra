...
func TestRead_Overall_Bowling_Summary_Stats(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "simple bowling summary read",
			args: args{
				ctx: ctx,
				db:  DB_POOL,
				queryMap: url.Values{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := Read_Overall_Bowling_Summary_Stats(tt.args.ctx, tt.args.db, tt.args.queryMap)
			if (err != nil) != tt.wantErr {
				t.Errorf("Read_Overall_Bowling_Summary_Stats() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
