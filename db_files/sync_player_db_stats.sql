...
ten_wicket_hauls AS (
    ...
    GROUP BY mw.bowler_id,
        mw.playing_format
),
best_bowling_match AS (
    SELECT DISTINCT ON (mw.bowler_id, mw.playing_format) mw.bowler_id,
        mw.playing_format,
        mw.total_wickets AS wickets,
        mw.total_runs AS runs
    FROM match_wickets mw
    ORDER BY mw.bowler_id,
        mw.playing_format,
    ...
    db_fc_stats = (
        SELECT combine_career_stats(
                ...
                (
                    ...
                    FROM match_stats ms
                    WHERE ms.player_id = p.id
                        AND ms.playing_format = 'first_class'
                )
            )
    ),
    db_lista_stats = (
