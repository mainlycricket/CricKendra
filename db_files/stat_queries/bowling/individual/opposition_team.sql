...
best_innings_fig_runs AS (
    ...
    FROM bowling_scorecards bs
        LEFT JOIN innings ON bs.innings_id = innings.id
        LEFT JOIN matches ON innings.match_id = matches.id
        LEFT JOIN bowling_performance bp ON bp.batting_team_id = innings.batting_team_id
        AND bp.bowler_id = bs.bowler_id
    WHERE matches.playing_format = 'ODI'
        AND matches.ground_id IN (63, 70, 79, 90, 124)
        AND matches.start_date >= '2008-08-18'
        AND matches.start_date <= '2024-09-27'
...
SELECT bs.bowler_id,
    ...
    innings.batting_team_id AS opposition_team_id,
...
FROM bowling_scorecards bs
    ...
    LEFT JOIN matches ON innings.match_id = matches.id
    LEFT JOIN bowling_performance bp ON innings.batting_team_id = bp.batting_team_id
    AND bs.bowler_id = bp.bowler_id
    LEFT JOIN match_stats ms ON ms.opposition_team_id = innings.batting_team_id
    AND bs.bowler_id = ms.player_id
    LEFT JOIN best_innings_fig_runs bifr ON bifr.batting_team_id = innings.batting_team_id
