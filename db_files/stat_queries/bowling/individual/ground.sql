...
best_innings_fig_runs AS (
    ...
    FROM bowling_scorecards bs
        LEFT JOIN innings ON bs.innings_id = innings.id
        LEFT JOIN matches ON innings.match_id = matches.id
        LEFT JOIN bowling_performance bp ON bp.ground_id = matches.ground_id
...
FROM bowling_scorecards bs
    ...
    LEFT JOIN bowling_performance bp ON matches.ground_id = bp.ground_id
    AND bs.bowler_id = bp.bowler_id
    LEFT JOIN match_stats ms ON ms.ground_id = matches.ground_id
    AND bs.bowler_id = ms.player_id
    LEFT JOIN best_innings_fig_runs bifr ON bifr.ground_id = matches.ground_id
