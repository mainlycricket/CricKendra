WITH match_stats AS (
    SELECT innings.id AS innings_id,
        COUNT(mse.player_id) AS players_count
...
FROM batting_scorecards bs
    ...
    LEFT JOIN matches ON innings.match_id = matches.id
    LEFT JOIN batting_performance bp ON bp.innings_id = innings.id
    LEFT JOIN match_stats ms ON ms.innings_id = innings.id
WHERE innings.is_super_over = FALSE
    AND matches.playing_format = 'ODI'
    AND innings.batting_team_id IN (1, 8, 10)
    AND innings.bowling_team_id IN (1, 8, 10)
