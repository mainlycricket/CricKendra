				...
				SELECT
					...
					ROW_NUMBER() OVER (
						...
						ORDER BY
							...
							bs.balls_bowled / 6 + bs.balls_bowled % 6 * 0.1 DESC, bs.bowling_position ASC
					) AS rn
				FROM
...
func ReadMatchFullScorecard(ctx context.Context, db DB_Exec, matchId int64) (responses.FullScorecardResponse, error) {
	var response responses.FullScorecardResponse
	matchHeader := &response.MatchHeader
								...
								SELECT
									ARRAY_AGG (
										...
										ROW (
											...
											bs.wickets_taken, bs.runs_conceded, bs.balls_bowled, bs.maiden_overs, bs.fours_conceded, bs.sixes_conceded, bs.wides_conceded, bs.noballs_conceded
										)
									)
								FROM
									bowling_scorecards bs
