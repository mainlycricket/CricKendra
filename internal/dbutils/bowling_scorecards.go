...
func InsertBowlingScorecardEntry(ctx context.Context, db DB_Exec, entry *models.BowlingScorecard) error {
	...
	cmd, err := db.Exec(ctx, query, entry.InningsId, entry.BowlerId, entry.BowlingPosition, entry.WicketsTaken, entry.RunsConceded, entry.BallsBowled, entry.MaidenOvers, entry.FoursConceded, entry.SixesConceded, entry.WidesConceded, entry.NoballsConceded)
