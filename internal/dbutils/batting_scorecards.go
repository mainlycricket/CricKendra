...
func InsertBattingScorecardEntries(ctx context.Context, db DB_Exec, entries []models.BattingScorecard) error {
	...
	for _, entry := range entries {
		_ = batch.Queue(query, entry.InningsId, entry.BatterId, entry.BattingPosition, entry.HasBatted, entry.RunsScored, entry.BallsFaced, entry.MinutesBatted, entry.FoursScored, entry.SixesScored, entry.DismissedById, entry.DismissalType, entry.DismissalBallNumber, entry.Fielder1Id, entry.Fielder2Id)
	}
